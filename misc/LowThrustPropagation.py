...
for propagator_index in range(number_of_propagators):
    ...
    for integrator_index in range(number_of_integrators):
        ...
        number_of_integrator_step_size_settings = 1
        ...
        for step_size_index in range(number_of_integrator_step_size_settings):
            ...
            # max_error_interpolator = interpolators.create_one_dimensional_vector_interpolator(state_history,
            #                                                                                   interpolators.lagrange_interpolation(8))
            # max_error = np.linalg.norm(max_error_interpolator.interpolate(list(function_evaluation_dict.keys())[-1]-20*86400*2)[0:3])
            # dict_to_write ['Max error'] = max_error
            function_time_dict = dynamics_simulator.cumulative_computation_time_history
            computation_time = list(function_time_dict.values())[-1]
            dict_to_write['Computation time'] = computation_time
            ...
            dict_to_write['Time requirement'] = (computation_time < 5)
...
states = np.loadtxt("./SimulationOutput/benchmarks/benchmark_1_states.dat")
dependent_var = np.loadtxt("./SimulationOutput/benchmarks/benchmark_1_dependent_variables.dat")
time = states[:,0]
...
ax.plot3D(states[:,1], states[:,2], states[:,3], label='Vehicle', linewidth=1.5)
ax.plot3D(dependent_var[:,4], dependent_var[:,5], dependent_var[:,6], label='Earth', linewidth=0.8)
ax.plot3D(dependent_var[:,7], dependent_var[:,8], dependent_var[:,9], label='Mars', linewidth=0.8)
