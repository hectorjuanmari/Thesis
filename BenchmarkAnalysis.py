...
benchmark_step_size_list = np.arange(0.25, 14.0, 0.25)*86400.0
max_error = np.zeros(np.size(benchmark_step_size_list))
i = 0

for benchmark_step_size in benchmark_step_size_list:
    ...
    benchmark_state_difference_norm = np.linalg.norm(benchmark_state_difference_list[:, 1:4], axis=1)
    max_error[i] = max(benchmark_state_difference_norm)
...
benchmark_state_difference_list = result2array(benchmark_dependent_difference)
...
ax.plot3D(first_benchmark_state_history_list[:,1], first_benchmark_state_history_list[:,2], first_benchmark_state_history_list[:,3], label='Vehicle', linewidth=1.5)
...
plt.figure(figsize=(width, height))
plt.plot(benchmark_step_size_list/86400.0, max_error, marker='.')
plt.axhline(y = 1e-1, color = 'tab:orange', linestyle = '--', linewidth=0.8,)
plt.axhline(y = 1e-2, color = 'tab:orange', linestyle = '--', linewidth=0.8,)
plt.yscale("log")
plt.grid(True, which="both", ls="--", linewidth=0.3)
# plt.grid(True, which="both", ls="dotted", color='0.8')
...
plt.plot(benchmark_state_difference_list[:,0], benchmark_state_difference_norm)
